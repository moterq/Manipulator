(window || this).bubbleText = function(ctrl) {
  ...
  function mapString(string) {
    ...
    for (var i = 0, l = string.length; i < l; i++) {
      ...
      array[i] = {
        char: char,
        html: $('<span class="span-bubble-text">' + char + "</span>").appendTo(
          $element
        )
      };
      if (char === " ") {
  ...
  spanCSS.height = /\d/.test(lineHeight)
    ? lineHeight
    : $(spans[0]).height() + "px";
  spans.css(spanCSS);
  ...
  var letterSpeed =
    parseInt(ctrl.letterSpeed) ||
    Math.floor((ctrl.speed || 2000) / animations.length);
  var boundaries = [" ", ".", ",", "-"];
